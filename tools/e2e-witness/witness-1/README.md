...
| [`0448c29562`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/0448c295625d96dc1fdda6de2d6f860f595c9bc5) | the lens overlay cannot be opened and none of its actions is registered | an action that is never registered leaves nothing a page can read, and the mojo bindings it guards are absent rather than refusing |
| [`c864878372`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/c864878372c58afc7e2314cb178727db911a9521) | no context menu offers to search or share a picture | a context menu is assembled at runtime and leaves nothing in the binary, and the strings behind those entries are still read by code that can no longer run |
| [`eb00d5c321`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/eb00d5c321a4e28a20ce0b963406f4909f080052) | the two untrusted lens pages are not served | an untrusted address with no configuration is refused at the top level with an invalid url error, which a harness page cannot navigate to and observe |
| [`586b1abdfd`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/586b1abdfdd1422dfcd06e4a692ddb5efd03356b) | no google password manager address is compiled in | the one address that has to stay, for a sync passphrase surface this fork has not closed, sits on the same host, and the two unique paths were already not being linked, so an absence check here could never have gone red |
| [`4fb3687abb`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/4fb3687abb9231869eba7c5f5dccce4dec9b828c) | no speech engine component is registered and the reading mode panel is not served | a component registration is a call made at start and leaves nothing in the binary, the update host it would have used is shared with components that remain, and an untrusted address with no configuration commits an empty document a harness page cannot observe |
...
| [`e986332e5c`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/e986332e5c) | typing the reading mode panel address does not terminate the browser | a harness page cannot drive a navigation to an untrusted address, and the crash it replaces was found by hand |
| [`2ae10e7f5b`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/2ae10e7f5b) | no tab builds reading mode and no side panel entry is registered for it | a side panel entry that is never registered leaves nothing a page can read, and the strings the controllers name are still read by the files that remain compiled |
| [`cdba06d4d9`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/cdba06d4d9) | no profile registers or syncs the fifteen reading mode settings | the preference names are still compiled in for the tree that remains, so their presence in the framework says nothing about whether a profile registers them, and what a profile writes is not something a page can read |
| [`5090eabba7`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/5090eabba7) | the media router answers that it is off and its two pages are not served | the cast button a page can ask for is drawn by the renderer only after the browser reports a screen as available, which a harness page cannot make happen, and a chrome page cannot be opened from one |
| [`b29a6c3840`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/b29a6c3840) | no profile builds an actor service and no tab builds its data object | a keyed service that is never constructed and a per tab object that is never built leave nothing a page can read |
| [`b265acfc66`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/b265acfc66) | the static eligibility answer is no and no profile builds a glic service | a keyed service that is never constructed is invisible to a page; the claim recorded here first, that the two glic pages read this same answer, was wrong, and the two commits below say what each of them actually reads |
| [`50041f0640`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/50041f0640) | opening settings does not terminate the browser and no profile reports itself eligible through the enablement struct | the crash it replaces was found by hand in a local crash dump, a chrome page cannot be opened from a harness page, and a struct that is only read inside the browser process leaves nothing a page can see |
| [`655b7eadaa`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/655b7eadaa) | the glic address is not served | a chrome address with no configuration is refused with an invalid url a harness page cannot navigate to and observe, and the renderer kill it replaces was found by hand |
...
| `cast-menu-strings` | [`be562726ad`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/be562726ad) | — | the cast menu title, the toolbar tooltip and the remoting toggle are not in the shipped resources |
| `cast-help-urls` | [`d6913afb61`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/d6913afb61) | — | the framework carries none of the three chromecast addresses the cast toolbar button's menu opened |
| `cast-interfaces` | [`4699da641c`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/4699da641c) | yes | the nine presentation and remote playback interfaces, the navigator attribute and the two media element attributes are all undefined |
...
| `closed-feature-hosts` | [`a019850d10`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/a019850d10ce9b9e5a8a736ce34f70f6f3e28417) | — | the framework carries no autofill server address and no translate script path |
...
| `geolocation` | [`99d4bb62b9`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/99d4bb62b93eff1d7aee3d75b03eab7a975d937c) [`61759f56f6`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/61759f56f6b970ffb918d300154cae71980e3919) | — | every position resolves to the origin of the grid |
...
| `lens-strings` | [`9eb58e519c`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/9eb58e519c) | — | none of the five lens texts that named a homework prompt, a region select instruction, an upload dialog, a composebox hint or a default query is in the shipped resources |
| `lens-upload-endpoints` | [`6146b4d3e2`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/6146b4d3e28621a4a52fc12c737ba278cc9a292e) | — | the framework carries no address the lens overlay uploaded pictures to |
...
| `reading-mode-tooltip` | [`ef070a9991`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/ef070a9991) | — | the tooltip that named all three reading mode actions is not in the shipped resources |
...
| `time-zone` | [`c82843dc63`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/c82843dc6318451d9a1fe989c670541bb9ca0ad3) [`ee66774f4f`](https://github.com/ARAS-Workspace/phantom-browser-core/commit/ee66774f4f3007d1076fa3ac9a327d69ec7bcb4d) | — | the zone is UTC while the browser runs elsewhere |
